    const WORKER_API_BASE = "http://localhost:5000";
    ...
    async function fetchWorkerDashboard() {
      try {
        const adharId = prompt("Enter Worker Aadhaar ID for testing");

        const response = await fetch(
          `${WORKER_API_BASE}/api/worker/me/${adharId}?month=2026-06`
        );
    ...
    async function submitLeave() {
      ...
      try {
        const workerAdhar = prompt("Enter Worker Aadhaar ID");

        const response = await fetch(
          `${WORKER_API_BASE}/api/leaves`,
          {
            method: "POST",
            headers: {
