...
async function renderMetrics() {
  try {
    const response = await fetch(
      "http://localhost:5000/api/supervisor/stats"
    );
    ...
    const metrics = [
      ...
      },
      {
        ...
        value: formatMoney(stats.total_wage),
...
function bindEvents() {
  ...
  try {
    ...
    console.error(err);
    showToast("Error loading leave requests");
  }
}
...
function renderLeaveRequests() {
  ...
  if (!leaveRequests.length) {
    body.innerHTML = `<tr><td colspan="8">No leave requests found.</td></tr>`;
    return;
  }

  body.innerHTML = leaveRequests
    .map(
      (leave) => `
      <tr>
        <td>${leave.worker_name}</td>
        <td>${leave.worker_adhar_id}</td>
        <td>${leave.job_cd}</td>
        ...
        <td>
          ${
            leave.status === "pending"
              ? `
                <button class="mini-btn approve leave-action"
                  data-id="${leave.id}"
                  data-status="approved">
                  Approve
                </button>

                <button class="mini-btn reject leave-action"
                  data-id="${leave.id}"
...
async function updateLeave(id, status) {
  const remarks = prompt(`Remarks for ${status} leave:`, "");
...
document.getElementById("refreshLeavesBtn")?.addEventListener("click", loadLeaveRequests);

loadLeaveRequests();

  document.body.addEventListener("click", (event) => {
    const leaveBtn = event.target.closest(".leave-action");

    if (leaveBtn) {
      const id = leaveBtn.dataset.id;
      const status = leaveBtn.dataset.status;
...
document.body.addEventListener("click", (e) => {
  const btn = e.target.closest(".leave-action");
